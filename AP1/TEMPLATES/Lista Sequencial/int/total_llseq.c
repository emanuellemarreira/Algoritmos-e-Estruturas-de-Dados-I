...
int busca_binaria_iterativa(Lista *minhalista, int valor){
	...
	int i, primeiro=0, meio=-1, ultimo=minhalista->qtd_elem_validos-1, comp=0;
	printf("\nBUSCA BINARIA ITERATIVA\n");
	while(primeiro<=ultimo){
		meio = primeiro + (ultimo-primeiro)/2;
		comp++;
		if(valor == minhalista->matriculas[meio]){
...
int main(){
	...
	printf("\ncheia: %d", cheia(l1));
	printf("\nvazia: %d\n", vazia(l1));
	printf("\nbusca seq ordenada: %d", buscaSeqOrd(l1,4));
	printf("\nbusca sequencial: %d", buscaSequencial(l1,4));
	printf("\nbusca binaria: %d", busca_binaria_iterativa(l1,4));
	printf("\nexcluindo do fim: ");
